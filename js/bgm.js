...
if (savedVolume !== null) {
  bgmAudio.volume = parseFloat(savedVolume);
  volumeSlider.value = savedVolume;

  const value = savedVolume * 100;
  volumeSlider.style.setProperty('--value', `${value}%`); // ✅ 막대도 반영
} else {
...
function playRandomButtonSound() {
  const rand = Math.floor(Math.random() * 4) + 1; // 3가지 중 랜덤
